...
void merge_part(int *arr,int n, int start1, int start2, int localN) {
    ...
    while (divisor <= thread_count) {
        ...
        if (rank % divisor == 0) { //Receiver
            ...
            merge_part(arr,start1,start2,localN);
        }
...
int main(int argc, char **argv) {
    ...
    int *globalArr = new int[n], *localArr = new int[n/thread_count];
    ...
    for (int i = 0; i < n; i++) localArr[i] = globalArr[i];
